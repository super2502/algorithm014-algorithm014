...
- 套路二： 二分查找，（最后发现这是一道证明题，证明模板的普适性，然后背就好了。。）
  ...
  - 二分变种1，单调数组，找头一个比目标小的鬼, 具体见表1
    ...
    - 不管什么情况， 拿right就对了
  - 二分变种2，单调数组但是有重复的，找到最左边的目标， 表2待整理
  - 二分变种3，搜索单调数组旋转过的
...
|2|l<r<target l<r\>target| 跟上边是一样的|这个细节处理 l<=mid加上l<=target<mid配套的太牛逼，直接把所有情况的target全部一脚踢到右边，这里有个问题，如果是找谁比target小就得瞎|
|3|其他情况已被标准情况cover|||
